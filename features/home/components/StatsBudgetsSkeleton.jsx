  ...
  return (
    <div className="rounded-xl bg-white px-5 py-6 md:p-8">
      ...
      <div className="grid gap-4 md:grid-cols-[1fr_auto] @max-[380px]:grid-cols-1">
        <div className="bg-grey-300/50 size-60 animate-pulse justify-self-center rounded-full" />

        <div className="grid grid-cols-2 gap-4 md:grid-cols-1 @max-[380px]:grid-cols-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="space-y-2">
              <div className="bg-grey-300/50 h-3.5 w-24 animate-pulse items-center rounded" />
              <div className="bg-grey-300/50 h-4 w-16 animate-pulse rounded" />
            </div>
          ))}
        </div>
      </div>
